...
def XOR(a, b): #(original plaintext , OTP)

    """Encryption using XOR, do not modify"""
    r = b""
    for x, y in zip(a, b): # a = char in plaintext, b = byte from byte array. 
        r += (x ^ y).to_bytes(1, "big") # character ascii value XOR byte value
    return r
...
def hax():
    otp = XOR(original_cipher, original_plaintext)
    new_plaintext = b"Student ID 100XXXX gets 4 points\n"
    new_cipher = XOR(new_plaintext,otp)
