...
pmt = [0, 16, 32, 48, 1, 17, 33, 49, 2, 18, 34, 50, 3, 19, 35, 51,
       ...
       12, 28, 44, 60, 13, 29, 45, 61, 14, 30, 46, 62, 15, 31, 47, 63] #new positions for each bits at old index i for i = [0,63]
...
def genRoundKeys(key):  #key starts from index 1. 
    K = [32, key]
    key_reg = key
    for i in range(1,32):

        #shift to left by 61
        new_key = rol(key_reg, 61, 80)

        #sbox the msb 4 bits
        left_nibble = sbox[(new_key >> 76) & 0xF]
        new_key &= ~(0xF << 76)
        new_key |= (left_nibble << 76)

        #xor the 15-19 bits with round
        xor_bits = (new_key >> 15) & 0b11111
        xor_bits ^= i
        new_key &= ~(0b11111<<15)
        new_key |= (xor_bits << 15) & (0b11111 << 15)

        key_reg = new_key
        round_key = ror(new_key, 16, 80) &  ((1 << 64) - 1)
        K.append(round_key)

    return K


def addRoundKey(state, Ki): #XOR operation between state and ki(round key)
    return state ^ Ki


def sBoxLayer(state): #do the state change thingy. 
    words_ls = []
    for i in range(16):
        value = (state >> (4*i)) & 0xF
        words_ls.append(sbox[value])
    words_ls.reverse()
    result = 0
    for words in words_ls:
        result = (result << 4) | words
    return result


def pLayer(state):
    final_state = [None] * 64 #final state is for new positions. 
    bit_ls = list(format(state, '064b'))  # Convert state to a 64-bit binary string


    for i in range(64): 
        final_state[pmt[i]] = bit_ls[i]

    final_val = int(''.join(str(bit) for bit in final_state), 2) 

    return final_val


def present_round(state, roundKey): #addroundkey, sbox, pLayer. 
...
def present_inv_round(state, roundKey):
    roundKey = roundKey & 0xffffffffffffffff
    # inverse pLayer
    before_state = [None] * 64
    bit_ls = list(format(state, '064b'))

    for i in range(64):
        before_state[i] = bit_ls[pmt[i]]

    after_pLayer  =  int(''.join(str(bit) for bit in before_state), 2)

    #inverse Sbox
    words_ls = []
    for i in range(16):
        value = (after_pLayer >> (4*i)) & 0xF
        words_ls.append(sbox.index(value))
    words_ls.reverse()
    ...
    final = roundKey ^ result 

    return final


def present(plain, key):
    K = genRoundKeys(key)       # initial state of key register, AFTER addRoundKey is applied to the key.
    state = plain               #initial plaintext
    for i in range(1, FULLROUND + 1):
        state = present_round(state, K[i]) #go through the stuff for each roud.
